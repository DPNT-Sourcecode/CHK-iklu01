    ...
    public static class CheckoutSolution
    {
        ...
        public static int GetProduct(string skus)
        {
            ...
            foreach (var c in arr)
            {
                ...
                if (countProducts[c] < 5 && DiscountedProducts.ContainsKey(c) && countProducts[c] % DiscountedProducts[c].ProductQuantity == 0)
                {
                    priceToPay -= DiscountedProducts[c].Discount;
                }

                //if (DiscountedProducts.ContainsKey(c) && countProducts[c] >= 5 & countProducts[c] % DiscountedProducts[c].ProductQuantity == 0)
                //{
